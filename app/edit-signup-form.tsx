...
export default function EditSignupFormScreen() {
  ...
  const { data: existingFields, isLoading: fieldsLoading } = useQuery({
    queryKey: ['signup-form-fields', formId],
  ...
  useEffect(() => {
    if (form && existingFields && !initialized) {
      ...
      if (form.deadline) {
        setHasDeadline(true)
        setDeadline(new Date(form.deadline))
      }
      setFields(
  ...
  }, [form, existingFields, initialized])
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      if (form?.event_id) {
        queryClient.invalidateQueries({ queryKey: ['event-signup-form', form.event_id] })
      }
      showToast('success', 'Signup form updated!')
      router.back()
    },
    onError: (error: any) => {
  ...
  const handleSubmit = () => {
    if (!formId) {
      showToast('error', 'Missing form ID')
      return
    }
  ...
  if (isLoading) {
    return (
      <View style={styles.container}>
        <Stack.Screen options={{ title: 'Edit Signup Form' }} />
      ...
      >
        <View style={styles.section}>
          ...
          <TextInput
            ...
            placeholder="Signup Form Title"
            placeholderTextColor="#9CA3AF"
            testID="edit-form-title-input"
          />
          ...
          <View style={[styles.switchRow, { marginTop: 18 }]}>
            <Text style={styles.switchLabel}>Form Active</Text>
            <Switch
              value={isActive}
              onValueChange={setIsActive}
              trackColor={{ false: '#D1D5DB', true: '#34D399' }}
              thumbColor={isActive ? '#059669' : '#F3F4F6'}
            />
          </View>
          {!isActive && (
            <Text style={styles.inactiveWarning}>
              Form is inactive — members won't see it or be able to sign up.
            </Text>
          )}
        </View>
